...
const NoteBlock: React.FC<{ note: Note }> = ({ note }) => {
  ...
  switch (note.type) {
    case 'info':
      ...
      bgColorClass = 'bg-blue-50 dark:bg-blue-900 dark:bg-opacity-30';
    ...
    case 'warning':
      ...
      bgColorClass = 'bg-yellow-50 dark:bg-yellow-900 dark:bg-opacity-30';
    ...
    case 'tip':
      ...
      bgColorClass = 'bg-green-50 dark:bg-green-900 dark:bg-opacity-30';
    ...
    case 'code':
        ...
        bgColorClass = 'bg-purple-50 dark:bg-purple-900 dark:bg-opacity-30';
...
const CodeExampleBlock: React.FC<{ example: { language: string; code: string; explanation?: string } }> = ({ example }) => {
  return (
    <div className="my-6">
      <div className="bg-slate-800 rounded-md shadow-lg overflow-hidden">
        <div className="px-4 py-2 bg-slate-700 text-slate-300 text-xs font-mono">
...
const ContentDisplay: React.FC<ContentDisplayProps> = ({ chapter, topic }) => {
  ...
  if (!topic || !chapter) {
    return (
      <div className="p-8 md:p-12 text-center min-h-screen flex flex-col justify-center items-center relative overflow-hidden bg-slate-900"> {/* Ensuring dark bg for welcome */}
        <div className="absolute inset-0 opacity-10"> 
          {/* Optional: Add a subtle background pattern or effect here if desired */}
        </div>
        <div className="relative z-10 flex flex-col items-center"> 
          <DigitalAlienIcon className="w-32 h-32 md:w-40 md:h-40 mx-auto text-sky-500 mb-8 animate-alienDanceAndHover" />
          <h2 className={`text-3xl md:text-4xl lg:text-5xl font-display font-semibold mb-6 uppercase ${gradientTextStyle} animate-textDance`}>Welcome to ZVC SMART CONTRACT MASTERY!</h2>
          <p className="text-lg text-slate-400 dark:text-slate-300 max-w-2xl mx-auto">
            Your comprehensive guide to mastering Smart Contracts, Web3, Blockchain, Hardhat, and Solidity.
          </p>
          <p className="text-md text-slate-500 dark:text-slate-400 max-w-xl mx-auto mt-4">
            Use the menu icon (top-left) to explore chapters and topics.
          </p>
          {/* Removed the "Getting Started" box for a cleaner welcome */}
        </div>
      </div>
    );
  }
  ...
  return (
    <article className="p-8 md:p-12 prose prose-slate dark:prose-invert max-w-none bg-slate-100 dark:bg-slate-900">
      <header className="mb-10 pb-6 border-b border-slate-300 dark:border-slate-700">
        ...
        <h1 className={`text-4xl font-display font-bold mt-2 ${gradientTextStyle}`}>{topic.title}</h1>
