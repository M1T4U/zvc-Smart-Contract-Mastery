...
const Sidebar: React.FC<SidebarProps> = ({ book, activeTopicId, onSelectTopic, onShowWelcome, isOpen }) => {
  const [openChapters, setOpenChapters] = useState<Record<string, boolean>>(() => {
    // Automatically open the chapter of the initially active topic if any
    if (activeTopicId) {
      for (const chapter of book.chapters) {
        if (chapter.topics.some(topic => topic.id === activeTopicId)) {
          return { [chapter.id]: true };
        }
      }
    }
    return {};
  ...
  const handleTopicSelection = (topicId: string, chapterId: string) => {
    onSelectTopic(topicId, chapterId);
    // Do not close sidebar here, App.tsx handles it.
  };
  ...
  return (
    <div 
      id="sidebar" // Added ID for ARIA
      className={`w-80 bg-slate-800 text-slate-200 p-6 space-y-6 min-h-screen fixed top-0 left-0 overflow-y-auto shadow-2xl z-20 
            ...
            {openChapters[chapter.id] && (
              <ul id={`chapter-topics-${chapter.id}`} className="mt-1 pl-4 space-y-1 border-l border-slate-700 ml-2">
                {chapter.topics.map((topic) => (
                  <li key={topic.id}>
                    <button
                      onClick={() => handleTopicSelection(topic.id, chapter.id)}
